...
const allowClone = (item: StateItem | null, user: UserItem | null): boolean => {
    ...
    const anyWrite = Object.values(user.permissions).some((x) => (x.value & MimirorgPermission.Write) === MimirorgPermission.Write);
...
const allowApproveCompany = (item: StateItem | null, user: UserItem | null): boolean => {
    ...
    const hasMinimumWrite = (permissionForCompany & MimirorgPermission.Write) === MimirorgPermission.Write;
    return hasMinimumWrite && item.state !== State.Delete && item.state !== State.Deleted && item.state !== State.ApproveCompany && item.state !== State.ApprovedCompany;
}
...
const allowApproveGlobal = (item: StateItem | null, user: UserItem | null): boolean => {
    ...
    const anyWrite = Object.values(user.permissions).some((x) => (x.value & MimirorgPermission.Write) === MimirorgPermission.Write);
    return anyWrite && item.state !== State.Delete && item.state !== State.Deleted && item.state !== State.ApproveGlobal && item.state !== State.ApprovedGlobal;
}