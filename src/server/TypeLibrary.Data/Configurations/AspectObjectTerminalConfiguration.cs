...
public class AspectObjectTerminalConfiguration : IEntityTypeConfiguration<AspectObjectTerminalLibDm>
{
    public void Configure(EntityTypeBuilder<AspectObjectTerminalLibDm> builder)
    {
        ...
        builder.HasOne(x => x.Terminal).WithMany(y => y.TerminalAspectObjects).HasForeignKey(x => x.TerminalId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(x => x.AspectObject).WithMany(y => y.AspectObjectTerminals).HasForeignKey(x => x.AspectObjectId).OnDelete(DeleteBehavior.Cascade);
    }
}