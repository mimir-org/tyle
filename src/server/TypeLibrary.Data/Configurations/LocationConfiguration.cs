...
namespace TypeLibrary.Data.Configurations
{
    public class AttributeTypeConfiguration : IEntityTypeConfiguration<AttributeTypeLibDm>
    {
        public void Configure(EntityTypeBuilder<AttributeTypeLibDm> builder)
        {
            ...
            builder.Property(p => p.AttributeTypeParentId).HasColumnName("ParentTerminalId").IsRequired(false);
            ...
            builder.HasOne(x => x.ParentAttributeType).WithMany(y => y.ChildrenAttributeTypes).HasForeignKey(x => x.AttributeTypeParentId).OnDelete(DeleteBehavior.NoAction);
        }
    }
}